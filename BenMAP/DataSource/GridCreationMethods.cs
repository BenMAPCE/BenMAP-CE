...
namespace BenMAP
{
	public partial class GridCreationMethods : FormBase
	{
		...
		public GridCreationMethods(BaseControlGroup bgc, string currentStat, string aqgPath)
		{
			InitializeComponent();
			_bgc = bgc;
			_currentStat = currentStat;
			_saveAQGPath = aqgPath;
			switch (currentStat)
			{
				case "baseline":
		...
		private void btnNext_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					ModelData frm = new ModelData(_bgc, _currentStat, _saveAQGPath);
					DialogResult rtn = frm.ShowDialog();
					if (rtn == DialogResult.Cancel) { return; }
					_strPath = frm.StrPath;
					_saveAQGPath = frm.SaveAQGPath;
					this.DialogResult = rtn;
				}
				else if (rbtnMonitorData.Checked)
				{
					MonitorData frm = new MonitorData(_bgc, _currentStat, _saveAQGPath);
					DialogResult rtn = frm.ShowDialog();
					if (rtn == DialogResult.Cancel) { return; }
