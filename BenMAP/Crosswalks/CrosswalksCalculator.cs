...
namespace BenMAP
{
    public static class CrosswalksCalculator
    {
        ...
        public static Dictionary<CrosswalkIndex, CrosswalkRatios> Calculate(IFeatureSet grid, IFeatureSet features, CancellationToken cancellationToken, IProgress progress)
        {
            try
            {
                ...
                Parallel.ForEach(features.Features, po, delegate(IFeature feature)
                {
                    ...
                    progress.OnProgressChanged(string.Format("Feature {0}/{1} finished", current, featuresCount), current / (featuresCount * 1.0f) * 100.0f);
                });
        ...
        public static Dictionary<CrosswalkIndex, CrosswalkRatios> Calculate2(IFeatureSet grid, IFeatureSet features, CancellationToken cancellationToken)
        {
            if (grid == null) throw new ArgumentNullException("grid");
            if (features == null) throw new ArgumentNullException("features");
