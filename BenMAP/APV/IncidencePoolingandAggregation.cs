                ...
                treeListView.Roots = lstRoot; this.treeColumnName.ImageGetter = delegate(object x)
  {
      if (((AllSelectCRFunction)x).NodeType == 100)
        ...
        void cbPoolingMethod_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            _operationStatus = 2; ((AllSelectCRFunction)cb.Tag).PoolingMethod = cb.Text;
            IncidencePoolingAndAggregation ip = CommonClass.lstIncidencePoolingAndAggregation.Where(p => p.PoolingName == tabControlSelected.TabPages[tabControlSelected.SelectedIndex].Text).First();
            ...
            for (int k = 0; k < i; k++)
            {
                switch (lstOLVColumns[k].Replace(" ", "").ToLower())
                {
                    ...
                    case "pollutant":
                        ...
                        break;
                    case "metric":
                        lstSecond = lstSecond.Where(p => (p.CRSelectFunction.BenMAPHealthImpactFunction.Metric==null ? "" : p.CRSelectFunction.BenMAPHealthImpactFunction.Metric.MetricName) == lstParent[i - k - 1]).ToList();
                        break;
                    case "seasonalmetric":
        ...
        public static List<string> getLstStringFromColumnName(string columName, List<CRSelectFunctionCalculateValue> lstCR)
        {
            ...
            switch (columName)
            {
                ...
                case "pollutant":
                    ...
                    break;
                case "metric":
                    lstString = lstCR.Select(p => (p.CRSelectFunction.BenMAPHealthImpactFunction.Metric == null ? "" : p.CRSelectFunction.BenMAPHealthImpactFunction.Metric.MetricName)).Distinct().ToList();
                    break;
                case "seasonalmetric":
                    ...
                    foreach (AllSelectCRFunction acf in queryTree)
                    {
                        ...
                        foreach (CRSelectFunctionCalculateValue crc in lstSecond)
                        {
                            ...
                            lstReturn.Add(new AllSelectCRFunction()
                            {
                                ...
                                Metric = crc.CRSelectFunction.BenMAPHealthImpactFunction.Metric == null ? "" : crc.CRSelectFunction.BenMAPHealthImpactFunction.Metric.MetricName,
