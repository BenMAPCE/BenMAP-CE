...
namespace BenMAP
{
    public partial class OnlineDatabaseImport : FormBase
    {
        public OnlineDatabaseImport()
        {
            InitializeComponent();
        }

        private void OnlineDatabaseImport_Load(object sender, EventArgs e)
        {
            GetOnlineSetups();
            SetupName();
        }
        ...
        private void GetOnlineSetups()
        {
            using (var client = new HttpClient())
            {
                ...
                List<fbFile> files = null;
                HttpResponseMessage response = client.GetAsync("BenMAPapi/api/fbFile").Result;
                if (response.IsSuccessStatusCode)
                {
                    
                    string jstring = response.Content.ReadAsStringAsync().Result;
                    files = JsonConvert.DeserializeObject<List<fbFile>>(jstring);
                    gvImportSetups.AutoGenerateColumns = false;
                    //gvImportSetups.Columns[0].DataPropertyName = "CreatedBy";
                    //gvImportSetups.Columns[1].DataPropertyName = "Company";
                    gvImportSetups.DataSource = files;
                }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            bool valid;
            int id = -1;
            fbFile download = new fbFile();
            try
            {
                DataGridViewRow row = gvImportSetups.SelectedRows[0];
                id = Convert.ToInt32(row.Cells[0].Value);
                if (importsetupID == 0)
                {
                    valid = false;
                    MessageBox.Show("Please select a setup to import to.", "Choose a Setup", MessageBoxButtons.OK);
                }
                else
                {
                    valid = true;
            ...
            {
                valid = false;
                MessageBox.Show("Please select a file to import.", "Choose a File", MessageBoxButtons.OK);
            }
            if (valid)
            {
                using (var client = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Accept.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    HttpResponseMessage response = client.GetAsync("BenMAPapi/api/fbFile/" + id.ToString()).Result;
                    if (response.IsSuccessStatusCode)
                    {
                        string jstring = response.Content.ReadAsStringAsync().Result;
                        download = JsonConvert.DeserializeObject<fbFile>(jstring);
                    }
                }
                ...
                //open byte[] to FileStream
                //download.Streamified
                using (MemoryStream stream = new MemoryStream(download.Streamified))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        ...
                        OnlineDatabaseImport_Load(sender, e);
                    }
                }
            }
        }
            ...
            try
            {
                ...
                for (int i = 0; i < tableCount; i++)
                {
                    ...
                    else
                    {
                        ...
                        //The 'F' is for the locked column in the SapeFileGridDefinitionDetails - this is imported and not predefined ---- MERGESIG
